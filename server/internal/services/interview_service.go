...
func (s *InterviewService) Delete(userID, interviewID uint) error {
	...
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("interview_id = ?", interviewID).Delete(&models.InterviewScore{}).Error; err != nil {
			return err
		}
		if err := tx.Where("interview_id = ?", interviewID).Delete(&models.InterviewReport{}).Error; err != nil {
			return err
		}
		return tx.Where("id = ? AND user_id = ?", interviewID, userID).Delete(&models.Interview{}).Error
	})
}
